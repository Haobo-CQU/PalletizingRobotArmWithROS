        ...
        while not rospy.is_shutdown():
            ...
            try:
                ...
                for i in circles[:]:
                    ...
                    cv2.circle(img, (i[0], i[1]), 2, (255, 255, 255), 2 * i[2] + 10)
                    # cv2.circle(img, (i[0], i[1]), 2, (255, 0, 255), 2)
                    # cv2.imshow("sdasscylinder", img)
                    # cv2.waitKey(0)
                # black circle
                where=np.where
                cir=np.zeros((720,1280),np.uint8)
                cv2.circle(cir,(640, 335),300, 255,-1)
                img[where(cir==0)]=0
                ...
                color_dist = {'red': {'Lower': np.array([0, 34, 153]), 'Upper': np.array([57, 240, 255])},
                              'blue': {'Lower': np.array([74, 160, 75]), 'Upper': np.array([110, 255, 255])},
                              'green': {'Lower': np.array([34, 46, 75]), 'Upper': np.array([76, 255, 255])},
