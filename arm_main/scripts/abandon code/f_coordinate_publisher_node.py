...
#           ####################################
#           适用于固定场景位置,不进行识别，直接给物块在摄像头中的坐标（单位：像素），进行Solve
#           ####################################
    ...
    def coordinate_identification_callback(self,data):
        ...
        while not rospy.is_shutdown():
            ...
            try:
                ...
                #                             1, 100, param1=100, param2=30, minRadius=20, maxRadius=200)
                ...
                # color_dist = {'red': {'Lower': np.array([168, 89, 5]), 'Upper': np.array([179, 255, 255])},
                #             'blue': {'Lower': np.array([35, 45, 5]), 'Upper': np.array([80, 255, 255])},
                #             'green': {'Lower': np.array([40, 137, 164]), 'Upper': np.array([163, 255, 255])},
                #             }# morning light without lamp
                                        
                # print("i come in, ok00?")
                # gs_frame = cv2.GaussianBlur(img, (7, 7), 1)  # 高斯模糊
                # hsv = cv2.cvtColor(gs_frame, cv2.COLOR_BGR2HSV)  # 转化成HSV图像
                # erode_hsv = cv2.erode(hsv, None, iterations=3)  # 腐蚀 粗的变细
                ...
                # list6 = [rect1[0][0], rect1[0][1], rect1[2]]
                # rect2 = cv2.minAreaRect(b)  # 长方形
                # if rect2[1][0] > rect2[1][1]:
                #     # print("Blue Cuboid x y theta", rect2[0], rect2[2])
                #     list7 = [rect2[0][0], rect2[0][1], rect2[2]]
                # else:
                #     # print("Blue Cuboid x y theta", rect2[0], -(90 - rect2[2]))
                #     list7 = [rect2[0][0], rect2[0][1], -(90 - rect2[2])]
                # box1 = cv.BoxPoints(rect1)  # 正方形
                # box2 = cv.BoxPoints(rect2)  # 长方形
                ...
                # list8 = [rect1[0][0], rect1[0][1], rect1[2]]
                # rect2 = cv2.minAreaRect(b)  # 长方形
                # if rect2[1][0] > rect2[1][1]:
                #     # print("Red Cuboid x y theta", rect2[0], rect2[2])
                #     list9 = [rect2[0][0], rect2[0][1], rect2[2]]
                # else:
                #     # print("Red Cuboid x y theta", rect2[0], -(90 - rect2[2]))
                #     list9 = [rect2[0][0], rect2[0][1], -(90 - rect2[2])]
                # box1 = cv.BoxPoints(rect1)  # 正方形
                # box2 = cv.BoxPoints(rect2)  # 长方形
                ...
                # data from image_proc
                list10 = [[712, 363, -45],[642.0, 152.0, -100.0], [696, 240.0, -90.00],
                 [787, 183, -150],[608,466, -110.0], [677,555, -25.80], 
                 [846, 328, 0], [794, 486, 0], [594, 320, 0]]
