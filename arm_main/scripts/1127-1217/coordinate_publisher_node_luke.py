...
def CoordinateIdentification():
    ...
    try:
        ...
        color_dist = {'red': {'Lower': np.array([136, 123, 157]), 'Upper': np.array([179, 199, 215])},
                        'blue': {'Lower': np.array([0, 159, 173]), 'Upper': np.array([124, 234, 255])},
                        'green': {'Lower': np.array([30, 43, 127]), 'Upper': np.array([81, 146, 255])},
        ...
        # opening = cv2.morphologyEx(erode_hsv,cv2.MORPH_OPEN,kernel)
        ...
        cnts = cv2.findContours(
            closing, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2]
